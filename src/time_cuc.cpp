...
void TimeCuc::get(uint8_t *pu8_TimeCucBuffer, const uint8_t u8_BufferSize, const TimeCuc::EFormat e_TimeCucFormat)
{
  uint8_t u8_CoarseTime;
  uint8_t u8_FineTime;
  uint64_t u64_Sec;
  ...
  for(uint8_t i=0; i<u8_CoarseTime; i++)
    pu8_TimeCucBuffer[1+i] = (uint8_t)(u64_Sec>>(8*(u8_CoarseTime-i-1)));
  for(uint8_t i=0; i<u8_FineTime; i++)
